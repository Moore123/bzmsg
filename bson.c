...
bson_iterator *alloc_iterator(){
    bson_iterator *retval = NULL;
    do { 
    if ( ( retval =calloc(1, sizeof(bson_iterator) ) ) == NULL ) break;
    } while(0);
    return(retval);
}

void free_iterator(bson_iterator *s){
    do { 
      if ( s == NULL ) break;
      free(s);
    } while(0);
...
int bson_ensure_space( bson *b, const size_t bytesNeeded ) {
    ...
    if ( pos + 2*bytesNeeded <= (size_t) b->dataSize )
        return BSON_OK;

    new_size = (int) ( 2 * ( b->dataSize + sizeof(char)*2*bytesNeeded ) );
